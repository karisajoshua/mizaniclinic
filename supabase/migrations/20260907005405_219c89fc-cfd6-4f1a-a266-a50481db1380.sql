...
BEGIN
  ...
  SELECT GREATEST(
    COALESCE(
      MAX(
        CASE
          WHEN ambassador_id ~ ('^MAP-' || country_code || '\d{4}[A-Z]{3}$')
          THEN CAST(SUBSTRING(ambassador_id FROM 6 FOR 4) AS INTEGER)
          ELSE 0
        END
      ), 0
    ),
  ...
  new_ambassador_id := 'MAP-' || country_code || LPAD(next_number::TEXT, 4, '0') || region_code;
...
SET ambassador_id = REPLACE(ambassador_id, 'MAP26-', 'MAP-')
WHERE ambassador_id LIKE 'MAP26-%';

UPDATE public.profiles
SET user_referral_id = REPLACE(user_referral_id, 'MAP26-', 'MAP-')
WHERE user_referral_id LIKE 'MAP26-%';